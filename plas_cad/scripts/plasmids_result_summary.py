...
def result_summary(ARGs, loc, ids, p_type):
    ...
    result_ids.close()
###################################### unmob plasmids ########################################
def result_summary_unmob(ARGs, loc, ids, p_type):
    result_ids = open(str(ids).rsplit("_",2)[0] + "_ids_result_out", "w")
    result_loc = open(str(loc).rsplit("_",2)[0] + "_loc_sum.txt" , "w")
    id_lis = {}
    loc_lis = []
...
if __name__ == "__main__":
    ...
    result_summary_unmob(str(args.i) + "_ARGs_annotation_out", str(args.i) + "_unmob_plasmids_loc_out", str(args.i) + "_unmob_plasmids_id_out", "unmob")
